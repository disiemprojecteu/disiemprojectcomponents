...
//added by csm, color
function drawPaths_noTransite(svg, cls, paths,color) {
...
//added by csm, color
function drawPaths_Transite(svg, cls, pathData,color,callback) {
    var paths = svg.selectAll('path.' + cls).data(pathData)
    paths.enter()
            .append('path')
            .classed(cls, true)
    paths.exit()
        ...
        // .style("stroke-opacity",0)
            .remove();
    var myPath = svg.selectAll('path.' + cls)
        .attr('d', makeD3Path)
    ...
    var nodes = myPath.nodes();
    var totalLength
    if(nodes && nodes.length){
        nodes.forEach(function(d){
            totalLength = d.getTotalLength();
            var cNode = d3.select(d)
            cNode.attr("stroke-dasharray", totalLength + " " + totalLength)
...
//added by csm, color
function drawPaths(svg, cls, pathData,color,callback) {
    var paths = svg.selectAll('path.' + cls).data(pathData,function(d){
        var arrayIndex = ""
        for(var i=0;i<d.length;i++){
            arrayIndex+=d[i][0]+"," + d[i][1]+" "
        }
        return arrayIndex
    })
    paths.enter()
            ...
            .each(function(d){
                ...
                totalLength = cNode.node().getTotalLength();                
                cNode.attr("stroke-dasharray", totalLength + " " + totalLength)
                  .attr("stroke-dashoffset", totalLength)
                  .transition()
                    .duration(5000)
